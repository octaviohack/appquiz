...
const evaluatePerformance = (correctAnswers, totalQuestions) => {
  const percentage = (correctAnswers / totalQuestions) * 100;
  if (percentage > 80) {
      return 'Congratulations! You did a great job.';
  } else if (percentage < 50) {
      return 'You need to study more. Keep trying!';
  } else {
      return 'Good effort! You can improve even more.';
  }
};

// ... within the return statement, in the result display section
 {showResult && (
  <div className="performance-evaluation">
      <p>{evaluatePerformance(result.correctAnswers, questions.length)}</p>
  </div>
   )}
